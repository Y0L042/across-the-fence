...
private _supplyDrops = configProperties [_gamemodeConfig >> "supplydrops"];
...
_officer setVariable ["vn_an_supply_drop_marker", _dropMarker, true];

/*// add supply officer actions
{
	private _request = configName _x;
	getArray (_gamemodeConfig >> "supplydrops" >> _request) params ["_requestName"];
	[_officer, _requestName, _request] call _fnc_add_action;
...
[[_officer, _officerMarker], {
	...
	[_officer] call vn_an_fnc_action_supplies;
